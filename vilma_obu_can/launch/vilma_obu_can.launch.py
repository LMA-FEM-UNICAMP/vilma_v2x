...
def generate_launch_description():
    ...
    vilma_obu_can = Node(
        package='vilma_obu_can',
        executable='vilma_obu_can_node',
        name='vilma_obu_can',
    ...
    # xsens launch
    parameters_file_path = Path(get_package_share_directory('vilma_obu_can'), 'param', 'xsens_mti_node.yaml')
    xsens_mti_node = Node(
    ...
    restart_on_node_exit = RegisterEventHandler(
        OnProcessExit(
            target_action=vilma_obu_can,
            on_exit=[
                EmitEvent(event=Shutdown(reason='vilma_obu_can died, shutting down launch'))
            ]
        )
    )
